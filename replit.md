...
- **Configurable AI retries**: `StorySettings.aiMaxRetries` (default 3, range 1–10) is sent as `maxRetries` in every ai-turn request body and validated by the `TriggerOpenrouterAiTurnBody` Zod schema. The API handler uses it as `maxAttempts`, overriding the `AI_MAX_ATTEMPTS` env var. Configurable via a slider in the AI Settings dialog.
- **RTL layout support**: `src/lib/rtl.ts` exports `isRtlLanguage(bcp47)` (checks primary subtag against a set of known RTL languages: ar, he, fa, ur, ps, ug, yi, dv, syr, ku). `src/hooks/use-document-dir.ts` exports `useDocumentDir(bcp47)` which sets `document.documentElement.dir` (`"rtl"` or `"ltr"`) and `document.documentElement.lang` reactively, enabling Tailwind's `rtl:*` variants globally. Both `home.tsx` and `story.tsx` call `useDocumentDir(settings.stt.aiLanguage)`. All directional utilities in story content use logical CSS properties (`ps-*`, `border-s-*`, `rounded-e-*`, `start-*`, `end-*`, `ms-*`, `me-*`) so they mirror automatically in RTL. Story text nodes (MessageBody, composition textarea, edit textarea, blind-mode draft preview) carry `dir="auto"` so mixed-language content renders with the correct base direction per-paragraph. `<TranslatedLine>` also uses logical properties and `dir="auto"` on its text div.
- **Cozy Storybook design**: All pages and dialogs use the warm storybook palette (coral `#E65C40`, amber `#FFB84D`, cream bg, polka-dot pattern). Fonts: Caveat for headings (via `index.html` + `.font-caveat` utility class), Nunito for UI, Lora for story text. CSS variables `--primary-shadow` (light `#C54A32`, dark `#8a3222`) power the 3D press effect on all primary buttons. All dialog titles use `font-caveat text-primary` by default via `ui/dialog.tsx`. Dark mode uses a warm night palette and all hardcoded hex colors in `home.tsx` have been replaced with `hsl(var(--card))` / `hsl(var(--card-foreground))` / `text-muted-foreground`.
- **AudioContext fix**: `use-sounds.ts` closes the AudioContext once after all oscillators finish via a single `setTimeout` (delay = total sound duration + 300 ms). This eliminates the `InvalidStateError: Cannot close a closed AudioContext` that occurred when multi-tone sounds (stt-complete, nudge, silence-warning) had their first oscillator's `onended` callback close the context while the second oscillator was still playing.
...
- **Settings Sheet**: All settings dialogs (TTS speed, voice, STT, AI/model, TTS play-order, language pickers, theme, debug toggle) are consolidated into a single slide-out Sheet triggered by a `<Settings>` icon in the header. The toolbar now shows only 4 always-visible icons (blind mode, auto/manual AI, play story, settings), keeping the header from overflowing on any screen size.
- **Sentence-highlight TTS (item 5)**: `MessageBody` groups tokens into sentence buckets delimited by `.!?؟`. The active bucket gets a sky-blue ring while the active word within it gets an amber highlight — giving both sentence-level and word-level context during TTS playback.
- **Merge adjacent paragraphs (item 6)**: Hovering between any two adjacent text paragraphs reveals a small merge button (`Merge` + `ChevronRight` icons). Clicking it PATCHes the first message with combined content and DELETEs the second.
- **AI image generation (item 4)**: A new violet `<ImageIcon>` button in the composer toolbar calls `POST /api/openrouter/conversations/:id/generate-image`. The endpoint (1) uses the text model to write a children's-book watercolour prompt from recent story context, (2) calls OpenRouter `/images/generations` with `black-forest-labs/flux-schnell:free`, (3) stores the result as an `assistant` message with `[STORYIMG]:url` prefix. The frontend renders these as inline `<img>` tags with a hover-delete button; TTS play-story skips image messages.
- **i18n**: All new features have English + Hebrew keys in `src/lib/i18n.ts`.
