...
cmd_logs() {
  ...
  tail -n "$lines" "$BACKEND_LOG" 2>/dev/null || echo "  (no log file yet: $BACKEND_LOG)"
  echo ""
  echo "  === CLIENT LOG (last ${lines} lines) ==="
  tail -n "$lines" "$CLIENT_LOG" 2>/dev/null || echo "  (no log file yet: $CLIENT_LOG)"
  echo ""
}

cmd_watch() {
  local interval="${2:-5}"
  echo "  Watching /api/healthz every ${interval}s on port $BACKEND_PORT. Ctrl+C to stop."
  echo ""
  while true; do
    local ts
    ts=$(date '+%H:%M:%S')
    local result
    result=$(curl -sf --max-time 3 "http://localhost:$BACKEND_PORT/api/healthz" 2>&1) || result="FAILED"
    echo "  [$ts] $result"
    sleep "$interval"
  done
}
...
case "${1:-help}" in
  ...
  help|--help|-h)
    echo ""
    echo "  Usage: ./init.app.sh {status|start|stop|restart|logs|watch}"
    echo ""
    ...
    echo "  logs     — tail the last 50 lines of both log files"
    echo "             (pass a number for a different line count, e.g. logs 100)"
    echo "  watch    — curl /api/healthz every 5 s (pass interval, e.g. watch 10)"
    echo ""
    echo "  Log files:"
    ...
    ;;
  *)
    echo "Unknown command: $1  (try: status | start | stop | restart | logs | watch)" >&2
    exit 1
    ;;
esac
